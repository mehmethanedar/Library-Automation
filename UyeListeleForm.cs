...
namespace VeritabaniProje
{
    public partial class UyeListeleForm : Form
    {
        ...
        {
            baglanti.Open();
            string kayit2 = "SELECT * FROM Kullanici where kullaniciTur = 'üye'";
        ...
        private void dataGridView1_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            kayit2 = "SELECT * FROM Okul where okulID = '" + okulID + "'";
            komut2 = new SqlCommand(kayit2, baglanti);
            baglanti.Open();
            dr = komut2.ExecuteReader();
            if (dr.Read())
            {
                comboBoxOkul.Text = dr["okulAdi"].ToString();
            }
            baglanti.Close();
        ...
        {
            baglanti.Open();
            string kayit1 = "update IletisimBilgileri set telefon=@telefon,adres=@adres,eposta=@eposta,ilID=@ilID,ilceID=@ilceID,okulID=@okulID where iletisimID = '" + id + "'";
            ...
            baglanti.Close();
            listele();
            MessageBox.Show("Üye Güncelleme İşlemi Gerçekleşti.");
        }
